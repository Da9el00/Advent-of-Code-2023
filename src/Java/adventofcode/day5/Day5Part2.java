...
public class Day5Part2 {
    ...
    protected void setupSeedStepData(){
        ...
        //Seeds
        seeds = Arrays.stream(inputSections.get(0).split(":")[1].trim().split(" ")).map(Long::parseLong).toList();
    ...
    public void findResult(){
        ...
        long result = LongStream.range(seeds.get(0) , seeds.get(0) + seeds.get(1)).boxed().toList().stream().map(this::performSteps).min(Long::compareTo).orElse(-1L);
